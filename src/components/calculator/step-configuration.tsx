...
function NumericInput({
  value, onChange, min = 0, step = 50, suffix, label,
}: {
  value: number; onChange: (v: number) => void; min?: number; step?: number; suffix?: string; label: string
}) {
  return (
...
export function StepConfiguration({
  ...
  function handleCapacityBlur() {
    ...
    if (!isNaN(v) && v > 0) onUpdateBeer({ capacity: v })
    else setBeerCapacityStr(String(config.beer.capacity))
  }
    ...
    <div className="flex flex-col gap-5">
      ...
      <div className="rounded-2xl border border-[hsl(var(--border))] bg-[hsl(var(--card-bg))] p-4 flex flex-col gap-3">
        ...
        <div className="grid grid-cols-3 gap-3">
          <NumericInput label="Hombre" value={config.carne.hombre} onChange={(v) => onUpdateCarne({ hombre: v })} step={50} suffix="g" />
          <NumericInput label="Mujer" value={config.carne.mujer} onChange={(v) => onUpdateCarne({ mujer: v })} step={50} suffix="g" />
          <NumericInput label="Niño" value={config.carne.nino} onChange={(v) => onUpdateCarne({ nino: v })} step={50} suffix="g" />
        </div>
      </div>
      ...
      <div className="rounded-2xl border border-[hsl(var(--border))] bg-[hsl(var(--card-bg))] p-4 flex flex-col gap-3">
        ...
        <div className="grid grid-cols-3 gap-3">
          <NumericInput label="Hombre" value={config.chorizo.hombre} onChange={(v) => onUpdateChorizo({ hombre: v })} step={50} suffix="g" />
          <NumericInput label="Mujer" value={config.chorizo.mujer} onChange={(v) => onUpdateChorizo({ mujer: v })} step={50} suffix="g" />
          <NumericInput label="Niño" value={config.chorizo.nino} onChange={(v) => onUpdateChorizo({ nino: v })} step={50} suffix="g" />
        </div>
      </div>

      {/* Cerveza: info + envase */}
      <div className="rounded-2xl border border-[hsl(var(--border))] bg-[hsl(var(--card-bg))] p-4 flex flex-col gap-3">
        <SectionTitle>🍺 Cerveza</SectionTitle>
        <div className="rounded-xl bg-[hsl(var(--secondary))] p-3 flex flex-col gap-1.5">
          <p className="text-xs font-semibold text-[hsl(var(--muted-fg))] uppercase tracking-wide">
            Niveles de consumo por persona
          </p>
          <div className="grid grid-cols-3 gap-2 mt-1">
            {[
              { label: 'Tranquilo', emoji: '🍺', liters: '1.5 L' },
              { label: 'Normal', emoji: '🍺🍺', liters: '2 L' },
              { label: 'Fuerte', emoji: '🍺🍺🍺', liters: '3 L' },
            ].map((item) => (
              <div key={item.label} className="flex flex-col items-center gap-0.5 rounded-lg bg-[hsl(var(--card-bg))] p-2 text-center">
                <span className="text-base">{item.emoji}</span>
                <span className="text-xs font-semibold">{item.label}</span>
                <span className="text-xs text-[hsl(var(--muted-fg))]">{item.liters}</span>
              </div>
            ))}
          </div>
          <p className="text-xs text-[hsl(var(--muted-fg))] mt-1">
            El nivel de cada participante se configura en su perfil de contacto o al editar el participante en este asado.
          </p>
        </div>
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="flex flex-col gap-1.5">
            ...
            <Select
              ...
              onValueChange={(v) => onUpdateBeer({ containerType: v as ContainerType, containerLabel: CONTAINER_TYPE_LABELS[v as ContainerType] })}
            >
              <SelectTrigger><SelectValue /></SelectTrigger>
              <SelectContent>
                {(Object.keys(CONTAINER_TYPE_LABELS) as ContainerType[]).map((t) => (
                  <SelectItem key={t} value={t}>{CONTAINER_TYPE_LABELS[t]}</SelectItem>
                ))}
              </SelectContent>
            </Select>
          </div>
          <div className="flex flex-col gap-1.5">
            ...
            <Select value={config.beer.unit} onValueChange={(v) => onUpdateBeer({ unit: v as CapacityUnit })}>
              <SelectTrigger><SelectValue /></SelectTrigger>
              <SelectContent>
      ...
      <div className="grid grid-cols-2 gap-3 sticky bottom-20">
        ...
        <Button onClick={onNext}>
          Ver lista <ChevronRight className="h-4 w-4" />
        </Button>
      </div>
      ...
      <Dialog open={resetOpen} onOpenChange={setResetOpen}>
        <DialogContent>
          ...
          <DialogFooter>
            ...
            <Button onClick={() => { onReset(); setBeerCapacityStr('473'); setResetOpen(false) }}>
              Restaurar
            </Button>
          </DialogFooter>
        </DialogContent>
